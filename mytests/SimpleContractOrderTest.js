...
contract('LimitOrderProtocol', async function ([_, wallet]) {
    ...
    beforeEach(async function () {
		...
		this.liquidator = await Liquidator.new(this.swap.address, ASSET_ADDRESSES.UNI);
		...
		await this.uni.methods.transfer(this.liquidator.address, '1000000').send({from: UNIAccountToImpersonate});
    ...
    describe('My test', function () {
        it('should swap uni', async function () {
			// Order: 1 UNI => 1 GUSD
			// Swap:  1 UNI => 1 GUSD
			const order = buildOrder(this.swap, this.uni, this.gusd, 1, 1, this.liquidator.address);
			...
			const makerUNI = await this.uni.methods.balanceOf(this.liquidator.address).call();
			console.log(`###: makerUNI`, makerUNI)
			const takerUNI = await this.uni.methods.balanceOf(_).call();
			const makerGUSD = await this.gusd.methods.balanceOf(this.liquidator.address).call();
			const takerGUSD = await this.gusd.methods.balanceOf(_).call();
			const receipt = await this.swap.fillOrder(order, signature, 1, 0, 1);
			const makerUNIAfter = await this.uni.methods.balanceOf(this.liquidator.address).call();
